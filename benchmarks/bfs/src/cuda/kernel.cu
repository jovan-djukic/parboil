...
struct LocalQueues {
	...
	int elements[NUM_BIN][W_QUEUE_SIZE];
	...
	__device__ void reset ( int index, dim3 block_dim ) {
	    ...
	    // few queues.
	    sharers[index] = ( block_dim.x >> EXP ) + ( threadIdx.x < ( block_dim.x & MOD_OP ) );
	}
	...
	__device__ void append ( int index, int *overflow, int value ) {
	    ...
	    if ( tail_index >= W_QUEUE_SIZE ) {
	    	*overflow = 1;
	    	...
	    	elements[index][tail_index] = value;
        }
	}
	...
	__device__ void concatenate ( int *dst, int ( &prefix_q )[NUM_BIN] ) {
	    ...
	    while ( local_shift < tail[q_i] ) {
	    	dst[prefix_q[q_i] + local_shift] = elements[q_i][local_shift];
...
// Process a single graph node from the active frontier. 
// Mark nodes and put new frontier nodes on the queue.
...
__device__ void visit_node ( int pid, int index, LocalQueues &local_q, int *overflow, int *g_color, int *g_cost, int gray_shade ) {
	...
	for ( int i = cur_node.x; i < cur_node.y + cur_node.x; i++ ) {
	    ...
	    // If this outgoing edge makes a shorter path than any previously discovered path
	    if ( orig_cost > cost ) {
...
//-------------------------------------------------
// This is the version for one-block situation. The propagation idea is basically the same as BFS_kernel.
...
//--------------------------------------------------
__global__ void BFS_in_GPU_kernel ( int *q1, int *q2, Node *g_graph_nodes, Edge *g_graph_edges, int *g_color, int *g_cost, int no_of_nodes, int *tail, int gray_shade, int k, int *overflow ) {
	...
	// propage through multiple BFS levels until the wavfront overgrows one-block limit
	while ( 1 ) {
	    ...
	    if ( tid < no_of_nodes ) {
	    	int pid;
	    	if ( tot_sum == 0 ) {
                // this is the first BFS level of current kernel call
	    	    pid = q1[tid];
            } else {
                // read the current frontier info from last level's propagation
	    	    pid = next_wf[tid];
            }

	    	// Visit a node from the current frontier; update costs, colors, and output queue
	    	visit_node ( pid, threadIdx.x & MOD_OP, local_q, overflow, g_color, g_cost, gray_shade );
	    }
        ...
        // the new frontier becomes empty;
        // BFS is over
	    if ( tot_sum == 0 ) {
	    	return;
        }
...
//--------------------------------------------------------------
__global__ void BFS_kernel_multi_blk_inGPU ( int *q1, int *q2, Node *g_graph_nodes, Edge *g_graph_edges, int *g_color, int *g_cost, int *no_of_nodes, int *tail, int gray_shade, int k, int *switch_k, int *max_nodes_per_block, int *global_kt, int *overflow ) {
    __shared__ LocalQueues local_q;
    __shared__ int         prefix_q[NUM_BIN];
	...
	// the total count of GPU global synchronization
	int kt = atomicOr ( global_kt, 0 ); 
 ...
 ***********************************************************************/
__global__ void BFS_kernel ( int *q1, int *q2, Node *g_graph_nodes, Edge *g_graph_edges, int *g_color, int *g_cost, int no_of_nodes, int *tail, int gray_shade, int k, int *overflow ) {
    __shared__ LocalQueues local_q;
    __shared__ int         prefix_q[NUM_BIN]; // the number of elementss in the w-queues ahead of
    __shared__ int         shift;             // current w-queue, a.k.a prefix sum
	...
	if ( tid < no_of_nodes ) {
	    // Visit a node from the current frontier; update costs, colors, and output queue
	    visit_node ( q1[tid], threadIdx.x & MOD_OP, local_q, overflow, g_color, g_cost, gray_shade );
	}
	__syncthreads ( );
	...
	if ( threadIdx.x == 0 ) {
	    ...
	    // the offset or "shift" of the block-level queue within the grid-level queue is determined by atomic operation
	    shift = atomicAdd ( tail, tot_sum );
	}
	__syncthreads ( );
