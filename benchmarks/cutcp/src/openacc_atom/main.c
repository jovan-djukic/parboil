...
Lattice* create_lattice ( LatticeDim dim ) {
    ...
    int size = ( ( dim.nx * dim.ny * dim.nz ) + 7 ) & ~7;
...
int main ( int argc, char *argv[] ) {
    ...
    Vec3 lo = ( Vec3 ) { min_ext.x - padding, min_ext.y - padding, min_ext.z - padding }; /* Bounding box with padding  */
    Vec3 hi = ( Vec3 ) { max_ext.x + padding, max_ext.y + padding, max_ext.z + padding }; /* Bounding box with padding  */
    printf("domain lengths are %g by %g by %g\n", hi.x - lo.x, hi.y - lo.y, hi.z - lo.z);

    LatticeDim lattice_dim = lattice_from_bounding_box ( lo, hi, h );
    Lattice *cpu_lattice = create_lattice ( lattice_dim );
    printf ( "\n" );

    /*
   * CPU kernel
   */
    if ( openacc_compute_cutoff_potential_lattice ( cpu_lattice, cutoff, atom ) ) {
