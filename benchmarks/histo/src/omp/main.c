...
int main ( int argc, char *argv[] ) {
	...
	unsigned char *bins = calloc ( numberOfThreads * histo_height * histo_width, sizeof ( unsigned char ) );

	for ( int iter = 0; iter < numIterations; ++iter ) {
		memset ( histo, 0, histo_height * histo_width * sizeof ( unsigned char ) );
		memset ( bins, 0, numberOfThreads * histo_height * histo_width * sizeof ( unsigned char ) );
		...
		for ( int i = 0; i < img_width * img_height; ++i ) {
			...
			unsigned char *myBin = bins + id * histo_width * histo_height * sizeof ( unsigned char );

			if ( myBin[value] < UINT8_MAX ) {
				++myBin[value];
			}
		}
		...
		for ( int i = 0; i < histo_width * histo_height; ++i ) {
			for ( int j = 0; j < numberOfThreads; ++j ) {
				int newValue = ( int ) histo[i] + bins[j * histo_width * histo_height + i];

				if ( newValue < UINT8_MAX ) {
					histo[i] = ( unsigned char ) newValue;
				} else {
					histo[i] = UINT8_MAX;
				}
			}
