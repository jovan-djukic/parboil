/* $Id: lbm.h,v 1.1 2008/03/04 17:30:03 stratton Exp $ */

/*############################################################################*/

#ifndef _LBM_H_
#define _LBM_H_

/*############################################################################*/

#include "config.h"

#include "lbm_1d_array.h"
/*############################################################################*/
/*
typedef enum {C = 0,
              N, S, E, W, T, B,
              NE, NW, SE, SW,
              NT, NB, ST, SB,
              ET, EB, WT, WB,
              FLAGS, N_CELL_ENTRIES} CELL_ENTRIES;
	      */
#define N_DISTR_FUNCS FLAGS

typedef enum {
    OBSTACLE    = 1 << 0,
    ACCEL       = 1 << 1,
    IN_OUT_FLOW = 1 << 2
} CELL_FLAGS;


/*############################################################################*/

void LBM_allocateGrid ( float** ptr );
void LBM_freeGrid ( float** ptr );
// void LBM_initializeGrid ( LBM_Grid grid );
// void LBM_initializeSpecialCellsForLDC ( LBM_Grid grid );
// void LBM_loadObstacleFile ( LBM_Grid grid, const char* filename );
// void LBM_initializeSpecialCellsForChannel ( LBM_Grid grid );
// void LBM_swapGrids ( LBM_GridPtr* grid1, LBM_GridPtr* grid2 );
// void LBM_performStreamCollide ( LBM_Grid srcGrid, LBM_Grid dstGrid );
// void LBM_handleInOutFlow ( LBM_Grid srcGrid );
// void LBM_showGridStatistics ( LBM_Grid Grid );
// void LBM_storeVelocityField ( LBM_Grid grid, const char* filename, const BOOL binary );
// void LBM_compareVelocityField ( LBM_Grid grid, const char* filename, const BOOL binary );

void LBM_initializeGrid ( float *grid );
void LBM_initializeSpecialCellsForLDC ( float *grid );
void LBM_loadObstacleFile ( float *grid, const char* filename );
void LBM_initializeSpecialCellsForChannel ( float *grid );
void LBM_swapGrids ( float **grid1, float **grid2 );
void LBM_performStreamCollide ( float *srcGrid, float *dstGrid );
void LBM_handleInOutFlow ( float *srcGrid );
void LBM_showGridStatistics ( float *Grid );
void LBM_storeVelocityField ( float *grid, const char* filename, const BOOL binary );
void LBM_compareVelocityField ( float *grid, const char* filename, const BOOL binary );

/*############################################################################*/

#endif /* _LBM_H_ */
