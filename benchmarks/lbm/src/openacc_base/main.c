...
int main ( int nArgs, char* arg[] ) {
	...
	#pragma acc data copy(srcGrid[-margin:(size + 2 * margin)]) copyin(dstGrid[-margin:(size + 2 * margin)])
	{
...
void MAIN_finalize ( const MAIN_Param* param ) {
	...
	LBM_freeGrid ( &srcGrid );
	LBM_freeGrid ( &dstGrid );
}
...
void MAIN_stopClock ( MAIN_Time* time, const MAIN_Param* param ) {
	...
	printf( 
	    ...
	    ( time->timeStop.tms_utime - time->timeStart.tms_utime + time->timeStop.tms_stime - time->timeStart.tms_stime ) * time->timeScale,
	    ( time->tickStop - time->tickStart ) * time->timeScale,
	    1.0e-6 * SIZE_X * SIZE_Y * SIZE_Z * param->nTimeSteps / ( time->tickStop - time->tickStart ) / time->timeScale 
	);
}
#endif
